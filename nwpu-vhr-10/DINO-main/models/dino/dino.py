...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        if decoder_sa_type == 'ca_label':
            ...
            for layer in [self.transformer.decoder.layer_1,self.transformer.decoder.layer_2]: #MODIFY
            # for layer in self.transformer.decoder.layers:
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        if attn_mask!= None:
            # try:
            if dn_meta['num_dn_group']!=1:
                bool_array = attn_mask[0]
                zero_positions = bool_array.nonzero(as_tuple=True)[0][-1]+1
                dn_data = {
            ...
            # except:
            #     print(dn_meta['num_dn_group'])
            #     print(attn_mask.shape)
            #     bool_array = attn_mask[0]
            #     zero_positions = bool_array.nonzero(as_tuple=True)[0]
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal,output_test = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask,dn_data)
        ...
        # output_test used to compute CKA similarity,if not, you could clean it
        output_test = []

        return out,output_test
