...
from .skip_encoder_decoder import SkipEncoderLayer,SkipDecoderLayer
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        if deformable_encoder:
            ...
            skip_layer = SkipEncoderLayer(d_model, dim_feedforward,
                                        dropout, activation,
                                        num_feature_levels, nhead, enc_n_points, add_channel_attention=add_channel_attention, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type)
        ...
        if deformable_decoder:
            ...
            skip_decoder_layer = SkipDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points, use_deformable_box_attn=use_deformable_box_attn, box_attn_type=box_attn_type,
                                                          key_aware_type=key_aware_type,
                                                          decoder_sa_type=decoder_sa_type,
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                        ...
                                        skip_decoder_layer=skip_decoder_layer,
                                        )
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None,dn_meta=None):
        ...
        # Begin Encoder
        #########################################################
        memory, enc_intermediate_output, enc_intermediate_refpoints,output_test1 = self.encoder(
                src_flatten, 
        ...
        # Begin Decoder
        #########################################################
        [hs, references],output_test2 = self.decoder(
        ...
        #########################################################        
        output_test1.extend(output_test2)

        return hs, references, hs_enc, ref_enc, init_box_proposal,output_test1
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        # prepare layers-----------------------------
        skip_flag = True
        self.skip_layer = [3,4,5]
        self.jump_layer = False
        if skip_flag:
            self.layer_1 = nn.ModuleList([copy.deepcopy(encoder_layer) for i in range(num_layers-len(self.skip_layer))]) #6-3 3
            self.layer_2 = nn.ModuleList([copy.deepcopy(skip_layer) for i in range(len(self.skip_layer))])  
        else:
            if num_layers > 0:
    ...
    def forward(self, 
        ...
        l = len(self.layer_1)+len(self.layer_2)
        jump = 0
        if len(self.layer_2)!=0:
            origin = self.skip_layer[0] #1
            end_i = self.skip_layer[-1] #5
            skip_layer_id =0
        ...
        for layer_id in range(l):
            # main process
            dropflag = False
            if layer_id in self.skip_layer:
        
                layer = self.layer_2[skip_layer_id] #5-1-2 = 2
                skip_layer_id +=1
                skip_f = True
                # print(f'skip {layer_id} - {origin}')
            else:
                if skip_f == True:
                    
                    layer = self.layer_1[origin_layer_id] #inter 4-5+0+ == -2
                    origin_layer_id+=1
                    # print(f'layer_1 {layer_id} - {end_i}+{origin_id}')
                    if self.jump_layer:
                        jump += 1
                    
                else:
                    layer = self.layer_1[origin_layer_id]
                    origin_layer_id += 1
                    # print(f'layer_1 {layer_id}')
            ...
            if not dropflag and layer_id not in self.skip_layer:
                if self.deformable_encoder:
                ...
                output = layer(src=output, pos=pos, reference_points=reference_points, spatial_shapes=spatial_shapes, level_start_index=level_start_index, key_padding_mask=key_padding_mask, skip_input=origin_output,skip_layer_id=skip_layer_id)   
        ...
        return output, intermediate_output, intermediate_ref,output_test
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.skip_layer = [2,3] 
        if self.skip_flag:
            self.layer_1 = nn.ModuleList([copy.deepcopy(decoder_layer) for i in range(num_layers-len(self.skip_layer))]) #6-3 3
            self.layer_2 = nn.ModuleList([copy.deepcopy(skip_decoder_layer) for i in range(len(self.skip_layer))])
        else:
            if num_layers > 0:
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id in range(l):
            # main process
            if layer_id in self.skip_layer:
                layer = self.layer_2[Skip_layer_id]
                Skip_layer_id += 1
                skip_f = True
                # print(f'decoder skip{layer_id}')
            else:
                if skip_f == True:
                    layer = self.layer_1[origin_layer_id] #3-5+1 = -1
                    origin_layer_id += 1
                    # print(f'decoder layer1 {layer_id} - {end_i} + {origin_id}')
                else:
                    if self.skip_flag:
                        layer = self.layer_1[origin_layer_id]
                        origin_layer_id +=1
                        # print(f'decoder layer1 {layer_id}')
                    else:
                        layer = self.layer_1[origin_layer_id]
                        origin_layer_id += 1
                        # print(f'decoder {layer_id}')
...
#             # nn.Conv2d(2*d_model, 2*d_model,kernel_size=1), # point
#             # nn.Conv2d(2*d_model,2*d_model,kernel_size=3, padding=1,groups = d_model//2), #depth
#             # nn.Conv2d(2*d_model, 2*d_model,kernel_size=1), # point
#             # nn.GELU(),
#         )
...
#         # src2 = self.with_pos_embed(src,pos) #new test
...
#         # skip-attention
#         self.skip_fc1 = nn.Linear(d_model,2*d_model)
#         # self.conv_2D = nn.Conv2d(2*d_model, 2*d_model, kernel_size=1)
#         # self.conv_point_1 = nn.Conv2d(2*d_model, 2*d_model,kernel_size=1) # query interior change information
#         # # self.conv_1D = nn.Conv1d(d_model,d_model,kernel_size=7,padding=3)
#         # self.conv_depth_1 = nn.Conv2d(2*d_model,2*d_model,kernel_size=3, padding=1,groups = 2*d_model) # 3 to 5，padding = 2
...
#                     # L,bs,d_model = noise_pos.shape
#                     # noise_pos = noise_pos.reshape(L//groups_temp, groups_temp, bs, d_model).transpose(0, 1)
#                     # noise_p = []
#                     # for i in noise_pos:
#                     #     noise_p.append(self.self_attn_noise(query=i,key=tgt_pos[dn_numbers:],value=tgt[dn_numbers:])[0]) # test
#                     # noise_pos = torch.cat(noise_p, dim=0)
...
#                 tgt2 = self.gelu(tgt2)
#                     # tgt2 = self.conv_2D(tgt2).squeeze(3).permute(0,2,1)
#                     # tgt2 = self.conv_depth_1(tgt2) #-->bs,2d,nq,1-->bs,4d,nq
#                     # # tgt2 = self.conv_1D(tgt2).unsqueeze(3) # bs,4d，nq -->  bs,4d,nq,1
#                     # tgt2 = self.conv_point_1(tgt2)
#                 tgt2 = self.block_(tgt2).squeeze(3).permute(0,2,1) # add residual

#                     # tgt2 = self.gelu(tgt2)
                    
#                     # tgt2 = self.conv_depth_2(tgt2).squeeze(3).permute(0,2,1) # bs,nq,2d
#                     # tgt2 = self.gelu(tgt2)
#                     # test add conv1d inversed bottleneck
